...
- [4. Wire Messages (Extensions to Existing JSON)](#4-wire-messages-extensions-to-existing-json)
  - [4.1 `swap_out_request` (JSON, type=42071) — Extension](#41-swap_out_request-json-type42071--extension)
  - [4.2 `swap_out_agreement` (JSON, type=42075) — Extension](#42-swap_out_agreement-json-type42075--extension)
  - [4.3 `swap_in_request` (JSON, type=42069) — Extension](#43-swap_in_request-json-type42069--extension)
  - [4.4 `swap_in_agreement` (JSON, type=42073) — Extension](#44-swap_in_agreement-json-type42073--extension)
- [5. Doing the Swap (Execution)](#5-doing-the-swap-execution)
- [6. `poll` Message Extension (Optional Proposal)](#6-poll-message-extension-optional-proposal)
- [7. Peer Discovery Strategies](#7-peer-discovery-strategies)
  - [Supplement (Clarification of Terminology)](#supplement-clarification-of-terminology)
- [日本語](#日本語)
  - [目的](#目的)
- [1. 動機](#1-動機)
- [2. ネットワークモデル](#2-ネットワークモデル)
- [3. 全体フロー](#3-全体フロー)
- [4. ワイヤメッセージ（既存 JSON の拡張）](#4-ワイヤメッセージ既存-json-の拡張)
  - [4.1 `swap_out_request`（JSON, type=42071）の拡張](#41-swap_out_requestjson-type42071の拡張)
  - [4.2 `swap_out_agreement`（JSON, type=42075）の拡張](#42-swap_out_agreementjson-type42075の拡張)
  - [4.3 `swap_in_request`（JSON, type=42069）の拡張](#43-swap_in_requestjson-type42069の拡張)
  - [4.4 `swap_in_agreement`（JSON, type=42073）の拡張](#44-swap_in_agreementjson-type42073の拡張)
- [5. Doing the Swap（実行）](#5-doing-the-swap実行)
- [6. `poll` メッセージの拡張（任意提案）](#6-poll-メッセージの拡張任意提案)
  - [方式](#方式)
- [7. ピア発見戦略](#7-ピア発見戦略)
  - [補足（読み替えの明確化）](#補足読み替えの明確化)
- [1. Motivation](#1-motivation)
- [2. Network Model](#2-network-model)
- [3. Overall Flow](#3-overall-flow)
- [4. Wire Messages (Extensions to Existing JSON)](#4-wire-messages-extensions-to-existing-json)
...
### 目的
**2 ホップ経路 `u → m → v`** でもスワップを可能にする**最小拡張**を定義する。
